// Full recipe detail page. Fetches a single recipe by ID and — for authenticated users — checks
// whether they have already saved it so the save button shows the correct state on load.
...
export default function RecipeDetailPage() {
  ...
  return (
    <Box minH="100vh" display="flex" flexDirection="column">
      ...
      {/* Main content */}
      <Box flex={1} maxW="900px" mx="auto" px={6} py={10} w="full">
        ...
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={10}>
          ...
          <Box>
            ...
            {recipe.instructions
              ...
              .map((step, i) => (
                <Box key={i} mb={3} fontSize="sm" color="gray.700" lineHeight="1.8">
                  <Text as="span" fontWeight="bold">Step {i + 1}: </Text>
                  {step.trim().replace(/^step\s*\d+[:\.\-]?\s*/i, '')}
                </Box>
              ))}
          </Box>
        </SimpleGrid>
